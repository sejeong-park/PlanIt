...
function createMarkerImage(markerSize, imageOrigin) {
  const markerImage = new kakao.maps.MarkerImage(imageOrigin, markerSize);
  return markerImage;
}
  ...
  ([focusLocation, isOpen]) => {
    ...
    if (focusLocation) {
      const newCenter = new kakao.maps.LatLng(
        focusLocation.data.latitude,
        focusLocation.data.longitude
      );
...
watch([() => props.planResultData, kakaoMapStatus], ([data, kakaoMapStatus]) => {
  if (!data) {
          ...
          if (index < array.length - 1) {
            ...
            if (nextTripInfo.latitude && nextTripInfo.longitude) {
              ...
              const nextLatLng = new kakao.maps.LatLng(
                nextTripInfo.latitude,
                nextTripInfo.longitude
              );
              const object = {
